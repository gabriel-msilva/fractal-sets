...
def _create_grid(xlim, ylim, number_points):
    x, y = np.ogrid[
        xlim[0] : xlim[1] : int(number_points) * 1j,  # noqa: E203
        ylim[0] : ylim[1] : int(number_points) * 1j,  # noqa: E203
    ]
...
def _apply_smoothing(count, z, max_iter):
    with warnings.catch_warnings():
        ...
        count = np.where(
            count < max_iter, count + 1 - np.log2(np.log2(np.abs(z))), count
        )
...
def plot_mandelbrot(
    ...
    **kwargs
    ...
    The Mandelbrot set is the set of complex numbers for which
    :math:`f_c(z) = z^2 + c` does not diverge when iterated from :math:`z = 0`.
...
def plot_julia(
    ...
    number_points: int = 300,
    ax=None,
    axis_labels: bool = "off",
    smoothing: bool = False,
    **kwargs
) -> Union[plt.Figure, plt.Axes]:
    """
    Plot the Julia set for complex quadratic polynomials.

    The family of complex quadratic polynomials are given by
    :math:`f_c(z) = z^2 + c`. The number of iterations is mapped to color.

    Parameters
    ----------
    c : complex
        Constant complex number of the logistic map,
    center : complex, default -0.5
        Center point of the plot.
    zoom : int, default 1
        Zoom ratio.
