...
from src.set_plotting import plot_mandelbrot, plot_julia
...
plt.savefig(images_dir.joinpath('julia_set.png'), 
            dpi=julia_args['number_points'] / 3.625, bbox_inches='tight')
...
plt.savefig(images_dir.joinpath('mandelbrot_set.png'), 
...
def draw_box(ax, center, zoom, color=None, **kwargs):
    ...
    ax.add_patch(
        Rectangle((center.real - a/2, center.imag - a/2), 
                  width=a, height=a,
                  edgecolor=color, fill=False, **kwargs)
        )
...
plt.savefig(images_dir.joinpath('mandelbrot_zoom.png'), 
            dpi=439, 
...
plt.savefig(images_dir.joinpath('mandelbrot_smoothing.png'), 
